...
def generate_for_alignment(
    ...
    eval_set = datasets.load_dataset('re-align/just-eval-instruct', split='test')
    # eval_set = random.sample(list(eval_set),2)
    eval_set = {
        **{key: [item[key] for item in eval_set] for key in eval_set[0].keys()},
        **{'question': [item['instruction'] for item in eval_set]}
    }
    ...
    eval_dict = eval_set.to_dict()
    eval_dict = [{k: v[i] for k, v in eval_dict.items()} for i in range(len(list(eval_dict.values())[0]))]
