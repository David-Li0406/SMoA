...
def convert_to_string(obj):
    ...
    if isinstance(obj, dict):
        return {str(key): convert_to_string(value) for key, value in obj.items()}
...
def generate_for_mmau(
    ...
    eval_set_org = []
    with open('data/{}_20240712.jsonl'.format(args.dataset)) as f:
        for line in f.readlines():
            eval_set_org.append(json.loads(line))
    
    sample_num = min(args.sample_num, len(eval_set_org))
    eval_set_org = random.sample(eval_set_org, sample_num)

    # preprocess for code_kaggle
    if args.dataset == 'code_kaggle':
        eval_set_org = convert_to_string(eval_set_org)

    eval_set = copy.deepcopy(eval_set_org)

    eval_set = {
        **{key: [item[key] for item in eval_set] for key in eval_set[0].keys()},
        **{'question': [item['messages'][0]['content'] for item in eval_set]}
    }
    ...
    eval_set_new = []
    for item_new, item_old in zip(eval_set, eval_set_org):
        eval_set_new.append({
            **item_old,
            **{"response": item_new["response"], 
            "generator": item_new["generator"], 
            "judge_output": item_new["judge_output"], 
            "chosen_responses": item_new["chosen_responses"], 
            "token_num_dict": item_new["token_num_dict"], 
            "total_round": item_new["total_round"],
            # "internal_result": item_new["internal_result"]
            }
        })

    with open(output_path, "w") as f:
        for item in eval_set_new:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, help='dataset to use', default='math_understand')
